...
- Advanced Search のURL共有は、圧縮形式 `?mode=advanced&qz=<deflate-raw + Base64URL JSON>` のみを扱う。
- encode/decode処理は `app/frontend/src/store/search/advancedSearchURL.ts` に集約する。URL生成時は `encodeConditionForCompressedURL()` を使い、常に圧縮して `qz` を発行する。
- `qz` はブラウザ標準の `CompressionStream` / `DecompressionStream` を使う。追加ライブラリは使わない。非対応環境や上限超過でエンコードできない場合はURLへ条件を載せず、history.stateへ退避する。
- Advanced Search のURL処理は非同期になっているため、初期復元や `popstate` 復元ではデコード完了後に検索を開始する。
- `qz` は共有URL専用の表現であり、Advanced Search のAPIリクエストは引き続きPOST bodyで送る。
- Simple Search のURL共有は、キーワードを `?mode=simple&term=<keyword>` として可読なまま残し、フィルタ条件だけを `filter=<deflate-raw + Base64URL JSON>` に圧縮して載せる。従来のフラットなURLクエリ（例: `dataset[jga_wgs]=0`）は互換性維持のため引き続き受け付ける。Simple Search の旧 `qz` URLは互換対象にしない。
- Simple Search の `term` / `filter` encode/decode処理は `app/frontend/src/store/search/simpleSearchURL.ts` に置く。URLに載せるフィルタ値はマスター定義との差分だけにし、APIリクエストは引き続きGETクエリとして送る。
- Simple/Advancedとも、圧縮パラメータ（Simpleは`filter`、Advancedは`qz`）に収まらない場合は、圧縮対象の条件をURLへ載せず `history.pushState`/`replaceState` の第1引数（`history.state`）へ退避する。Simple Search の `term` は可読なままURLに残すが、`term`自体が単独の上限（`SIMPLE_SEARCH_TERM_MAX_LENGTH`、`simpleSearchURL.ts`）を超える場合はURLから省き、`filter`と同様にhistory.stateへ退避する。共有URL自体には圧縮対象の条件が載らないが、同一ブラウザでの戻る/進む・リロードでは `window.history.state` から条件を復元できる。この場合 `searchManager.ts` が共通の `searchURLTooLong` フラグをStoreへセットし、`api/searchMessages.ts` の `mergeSearchURLTooLongWarning()` が `searchURLRestoreWarning` と同じ経路（`searchMessages.warning` → 検索結果画面のメッセージ表示）で警告を合成する。history.stateへの退避と読み取りは `store/search/searchURL.ts`（退避）・`store/search/searchHistory.ts`（復元）・`store/search/searchURLCodec.ts` の `getObjectFromHistoryState()` に集約する。
- URLから復元した条件をViewへ戻す処理は `components/AdvancedSearch/AdvancedSearchConditionRestorer.ts` を確認する。
- `qz` は `+` `/` `=` を含まないBase64URLとして生成するため、生Base64と違いURL中で壊れない。URLへの埋め込みは `qs.stringify()` が担うため、個別に `encodeURIComponent` を呼ぶ必要はない。
...
- 検索条件のURL反映や初期復元は `app/frontend/src/store/search/searchManager.ts` と `app/frontend/src/store/search/initializeApp.ts` を確認する。
